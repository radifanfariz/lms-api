...
DROP SEQUENCE t_gallery_data_n_id_seq;
CREATE SEQUENCE t_gallery_data_n_id_seq INCREMENT BY 1 MINVALUE 1 MAXVALUE 2147483647 START WITH 1  NO CYCLE;
...
CREATE TABLE t_access_data (n_id INTEGER NOT NULL GENERATED ALWAYS AS IDENTITY, n_module_meta_id INTEGER, c_created_by CHARACTER VARYING, c_updated_by CHARACTER VARYING, d_created_at TIMESTAMP(6) WITHOUT TIME ZONE, d_updated_at TIMESTAMP(6) WITHOUT TIME ZONE, c_global_id CHARACTER VARYING, n_array_grade_id INTEGER[], n_array_user_id INTEGER[], PRIMARY KEY (n_id));
DROP TABLE t_gallery_data;
CREATE TABLE t_gallery_data (n_id INTEGER NOT NULL GENERATED ALWAYS AS IDENTITY, c_name CHARACTER VARYING, c_src CHARACTER VARYING, c_created_by CHARACTER VARYING, c_updated_by CHARACTER VARYING, d_created_at TIMESTAMP(6) WITHOUT TIME ZONE DEFAULT now(), d_updated_at TIMESTAMP(6) WITHOUT TIME ZONE DEFAULT now(), n_user_id INTEGER, PRIMARY KEY (n_id));
...
ALTER TABLE "t_access_data" ADD CONSTRAINT taccessdata_fk1 FOREIGN KEY ("n_module_meta_id") REFERENCES "t_module_metadata" ("n_id");
ALTER TABLE "t_gallery_data" ADD CONSTRAINT tgallerydata_fk1 FOREIGN KEY ("n_user_id") REFERENCES "t_user_data" ("n_id");
